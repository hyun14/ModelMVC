...
<div style="width:98%; margin-left:10px;">
  ...
  <form name="detailForm" method="post" action="${ctx}/listProductEntry.do">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;" class="table-list">
    ...
    <c:forEach var="product" items="${list}">
      ...
      <c:url var="prodDetailUrl" value="${ctx}/getProduct.do">
		  <!-- 기존 파라미터 -->
	  ...
	  <tr class="ct_list_pop click-row" data-mode="${menuVal}" data-href="${prodDetailUrl}">
        ...
        <td align="left" class="name-cell">
          <c:choose>
            ...
            <c:otherwise>
              <%-- <a href="${ctx}/getProduct.do?menu=${menuVal}&prodNo=${product.prodNo}&code=${code}"> --%>
                <span class="ellipsis">${product.prodName}</span>
              <!-- </a> -->
            </c:otherwise>
          </c:choose>
        ...
        <td class="status-cell">
          ...
          <c:choose>
            ...
            <c:when test="${user.role == 'admin' and menuVal == 'manage'}">
              ...
              <c:if test="${code == 'BEF'}">
                &nbsp;|&nbsp;
                <a class="status-link" href="${ctx}/updateTranCodeByProd.do?prodNo=${product.prodNo}&page=${search.currentPage}">배송하기</a>
              </c:if>
            </c:when>
