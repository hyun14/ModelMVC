...
public class ProductRestController {
    ...
    public List<String> suggest(
        ...
        return productService.suggestProducts(q, limit);
    }
    ...
    public Map<String, Object> listJson(
        ...
        // 기본값 보정
        int cp = (search.getCurrentPage() > 0) ? search.getCurrentPage() : 1;
        int ps = (search.getPageSize() > 0) ? search.getPageSize() : pageSize;
        search.setCurrentPage(cp);
        search.setPageSize(ps);

        Map<String, Object> result;
        if ("manage".equals(menu)) {
            // 필요 시 관리자 전용 목록으로 분기 (프로젝트 컨벤션에 맞게)
            result = productService.getProductList(search);
        } else if ("search".equals(menu)) {
            // 사용자 검색 목록이 따로 있으면 여기로
            result = productService.getProductListByUser(search);
        } else {
            result = productService.getProductList(search);
        }

        Map<String, Object> res = new HashMap<>();
        res.put("list", result.get("list"));
