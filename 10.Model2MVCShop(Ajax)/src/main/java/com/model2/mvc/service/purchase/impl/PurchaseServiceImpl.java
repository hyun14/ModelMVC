...
public class PurchaseServiceImpl implements PurchaseService {
	...
	public Map<String, Object> getPurchaseList(String buyerId, Search search) throws Exception {
		...
		// 1. 기존 로직으로 구매 목록을 먼저 조회합니다.
	    Map<String, Object> map = purchaseDao.getPurchaseList(buyerId, search);
	    List<Purchase> list = (List<Purchase>) map.get("list");

	    // 2. [추가] 조회된 목록을 반복하며 각 상품에 이미지 정보를 추가합니다.
	    for (Purchase purchase : list) {
	        Product product = purchase.getPurchaseProd();
	        if (product != null) {
	            // ProductDao를 사용해 상품 이미지를 조회하고 설정합니다.
	            product.setImages(productDao.selectProductImages(product.getProdNo()));
	        }
	    }

	    // 3. 이미지 정보가 포함된 최종 목록을 반환합니다.
	    return map;
	}
